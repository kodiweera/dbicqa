package BXHPerlUtils::XMLMetadata;
...
sub start_element {
  ...
  if ($localname eq 'dimension' && !$self->{'indimension'}) {
	return if $pathref->[$#$pathref-1] ne 'datarec';
...
sub end_document {
  my ($self,) = @_;
  my @serializable_fields = ( 'dims', 'bvalues', 'elementtype', 'sliceorder', );
  for my $key (keys %{$self}) {
    if (! grep { $key eq $_ } @serializable_fields) {
      delete $self->{$key};
    }
  }
}
sub dump {
  my ($self,) = @_;
  # "unbless" the structure so JSON doesn't have a conniption
