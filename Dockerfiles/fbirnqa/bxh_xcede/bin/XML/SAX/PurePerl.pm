# $Id: PurePerl.pm,v 1.1 2009-04-03 00:58:33 gadde Exp $
    ...
    if ($data =~ /^<!--/) {
        ...
        while (1) {
            ...
            if ($data =~ /^(.*?)-->/s) {
                ...
                # XXX [SG]: it's possible close comment marker might occur
                # across the current and next buffers, so add this check
                my $movelen = length($data);
                if ($data =~ /(-+)$/) {
                    $movelen -= length($1);
                }
                $comment_str .= substr($data, 0, $movelen);
                $reader->move_along($movelen);
            }
        }
